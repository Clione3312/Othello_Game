...
public class Difficulity_Normal
{
    ...
    public string GetSpritePosition(int[][] field, int ySprite, int eSprite) {
        ...
        // 置ける位置を取得する
        string posStr = GetSpriteCheck.I.PutSpritePosition(fieldState, yourSprite, enemySprite);
        ...
        foreach (string item in posStr.Split(' '))
        {
            ...
            // 盤面をコピー
            int[][] cloneField = Difficulity_Public.I.CreateCloneField(fieldState);

            // 特定の位置から、獲得できる駒位置を取得する
            string getSprite = GetSpriteCheck.I.GetSpritePosition(posY, posX, cloneField, yourSprite, enemySprite);
            int[][] getPosArray = GetSpriteCheck.I.ConvertJuggedArray(getSprite);
            ...
            float currScore = NegaMaxMethod(cloneField, DEEP_LEVEL - 1, enemySprite, yourSprite, ref maxScore);
    ...
    private float NegaMaxMethod(int[][] field, int limit, int ySprite, int eSprite,ref float maxScore) {

        // 両サイドの
        string yPutPos = GetSpriteCheck.I.PutSpritePosition(field, ySprite, eSprite);
        string ePutPos = GetSpriteCheck.I.PutSpritePosition(field, eSprite, ySprite);
        ...
        if (yPutPos != string.Empty) {
            ...
            foreach (string item in yPutPos.Split(' '))
            {
                ...
                // 盤面をコピー
                int[][] cloneField = Difficulity_Public.I.CreateCloneField(field);

                // 特定の位置から、獲得できる駒位置を取得する
                string getSprite = GetSpriteCheck.I.GetSpritePosition(posY, posX, cloneField, ySprite, eSprite);
                int[][] getPosArray = GetSpriteCheck.I.ConvertJuggedArray(getSprite);
                ...
                // 次のターンの処理を実行する
                float currScore = NegaMaxMethod(cloneField, limit - 1, eSprite, ySprite,ref maxScore) * -1;
                if (currScore < maxScore){
                    maxScore = currScore;
            ...
            // 次のターンの処理を実行する
            float currScore = NegaMaxMethod(field, limit - 1, eSprite, ySprite,ref maxScore) * -1;
            if (currScore < maxScore) {
                maxScore = currScore;
            } else if (currScore > maxScore) {
                // 置ける場所がない場合、評価値を返す
